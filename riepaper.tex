...
%
\author{Tim Wood\inst{1} \and Shuvendu K. Lahiri\inst{2} \and Sophia Drossopolou\inst{1} \and Susan Eisenbach\inst{1}}
%
...
In this work, we present RIE, a refined methodology to modularly establish equivalence of procedures in the presence of memory allocation, cyclic data structures and recursion. Our technique addresses the need for finding witnesses to isomorphism with angelic allocation, supports reasoning about equivalent procedures calls when the stores are only locally isomorphic, and reasoning about changes in the order of procedure calls. We have implemented RIE by encoding it in the Boogie program verifier. We describe the encoding and prove its soundness.

% Sophia's version

%In the course of program maintenance, one often replaces a procedure by an equivalent version, where two procedures are equivalent, if, when called on isomorphic states, they result in isomorphic states. Given the prevalence of maintenance in the program lifecycle, automatic verification of procedure equivalence is of great practical importance. Current tools are very powerful in establishing a stronger version of equivalence, \ie that two procedures when applied to equal states  result in equal states. However, in the presence of dynamic memory allocation strong equivalence is too strong, and the tools cannot be applied.

%We have developed a methodology, \metho{}, which can automatically establish (weak) equivalence of procedures in the presence of memory allocation as well as recursion. For this, we faced several challenges, among which the need to search for a witness to the isomorphism, and the fact that corresponding nested method calls do not necessarily take place on equivalent states. We addressed the former by searching instead for an angelic allocator, and the latter, by adapting the mutual summaries approach so that it only requires the accessible part of the state to be equal and only guarantees that the modified part of the resulting states are equal.

%We have implemented \metho{} in a prototype tool, \tool{}, which produces Boogie code, and ultimately is solved by the SMT solver Z3. In encoding reachability, heap equality, and equality up to reachability we had to combine uninterpreted functions, and triggers so as to control unfolding of the predicates.

%In this paper we describe \metho{}, the Boogie encoding and prove soundness.

% My version

%Program maintenance dominates the lifecycle but often leads to unwanted changes in behaviour. The testing community recognise this with the use of regression testing. Advances in constraint solving and other technologies are making fully automatic program equivalence verification more practical, and recently several equivalence verification tools have been developed. They have limited support for the heap, with dynamic heap memory allocation a particular problem. We present a methodology and tool, for a simple OO programming language, that can prove equivalence of programs where heap allocation differs. Our modular verification methodology is sound and amenable to an SMT solver.

\keywords{Program Equivalence, Program Verification, Version-aware Verification}
\end{abstract}

\section{Introduction}
%fixing bugs and adding features is recurring, expensive, and error-prone

Program maintenance dominates the program lifecycle. A study of application bugs that took more than one attempt to fix~\cite{Park2012} found that 22\%-33\% of fixes required a supplementary fix, and found a diverse range of errors including incomplete refactorings. A study of refactorings~\cite{Bavota2012} found that, across 12,922 refactorings from three software projects, 15\% of refactorings induced a bug. \emph{Automatic program equivalence verification}~\cite{Godlin09,Lahiri2012,Partush2014,Felsing2014} offers the potential to reduce problems by allowing a programmer to automatically (without programmer annotations) verify that the new version is, fully or partially, behaviourally equivalent to the old.

% A study of operating system bugs~\cite{Yin2011} found that at least 14.8\%-24.4\% of patches were incorrect. 

The goal of these verification tools is to make the benefits of program equivalence verification available to programmers who are not verification experts. An automatic equivalence verification tool \emph{takes a pair of programs as input and then outputs whether the programs are equivalent or not} (or perhaps times-out). Program equivalence is undecidable in the general case, however, some success has been achieved on programs with substantially similar structure. Different versions of a software product are frequently structurally similar because a later version is often created by modifying an earlier version. 

We know of four existing tools designed with the goal of fully-automatic program equivalence verification: Symdiff~\cite{Lahiri2012}, RVT~\cite{Godlin09}, SCORE~\cite{Partush2014}, and R{\^e}ve~\cite{Felsing2014}. SCORE~\cite{Partush2014}, and R{\^e}ve~\cite{Felsing2014} support numerical, rather than heap programs. Symdiff and RVT have some built-in support for heap manipulating programs. Symdiff~\cite{Kawaguchi2010,Lahiri2012,Lahiri2013,Hawblitzel2013} is built on top of the Boogie~\cite{Barnett2005} intermediate verification language which, in turn, uses the Z3~\cite{DeMoura2008} satisfiability modulo theories (SMT) solver to discharge proof obligations. RVT uses a designed-for-purpose verification algorithm, which passes program fragments to the CMBC~\cite{Clarke2003} bounded model checker. 
...
\textbf{\Ctwo{}}. A tool needs to determine when stores correspond in the footprint of a called procedure.

\textbf{\Cthree{}}. Equivalent calls from non-isomorphic stores do not necessarily result in isomorphic stores.

\textbf{\Cfour{}}. It is not known in advance which calls may be equivalent, and there may be many candidates. Equivalent calls may not occur in the same order in each procedure, and moreover the procedure calls which correspond may differ from execution to execution depending on the initial state (\ie program inputs).

%\textbf{Challenge 5}. Because the question of verifying program equivalence is undecidable in general, and has high computational complexity in many specific cases, we had to design our Boogie encoding carefully so that predicates are unfolded enough but not too much.
...
We give an example of a pair of equivalent procedures that differ in the order of memory allocation, then discuss why they are not \symdiffequiva{} (for example in Symdiff, the only tool that handles this example soundly). We motivate the need for a proof system to establish they are equivalent.
...
\end{lstlisting}%
\end{minipage}%
\caption[Two procedures, \copylr{} and \copyrl{}, that copy a binary tree]{Two procedures that copy a binary tree. Procedure \copylr{} copies the left nodes first and then the right nodes, and allocates the parent node before copying the children. Procedure \copyrl{} copies the right nodes and then the left nodes, and allocates the parent node after copying the children. The postcondition \texttt{modifies \{r\}} asserts that no existing object, other than \texttt{r}, is modified. For this example \tool{} requires this framing assertion. Our approach can take advantage of any contracts that are available.\label{fig:copy tree}}%
...
\Cref{fig:copy tree} shows recursive procedures \copylr{} and \copyrl{}. Both procedures are intended to recursively copy a tree passed as parameter \texttt{t}. The procedures are equivalent on \emph{any} input, whether the input is tree shaped of not. \metho{} and \tool{} can discover that. The procedures differ in two ways: Firstly, the allocation of the copied node has been moved from before the recursive calls on \cref{lst:orig copylr parent} to after the recursive calls on \cref{lst:orig copyrl parent}. Secondly, the order of the recursive calls on \cref{lst:orig copylr call1,lst:orig copylr call2} has been reversed on \cref{lst:orig copyrl call1,lst:orig copyrl call2}. The procedures are written in a simple language we call \lang{}, formalised in \cref{sec:soundness}.

The procedures are equivalent. An intuition as to why is: when \texttt{t} or \texttt{r} are \texttt{null} both procedures leave the heap unchanged. Otherwise, both procedures recursively copy all the nodes to the left, and all the nodes to the right, and return a newly allocated root node via the parameter \texttt{r}. The only pre-existing object modified by the procedures is the one pointed to by \texttt{r}. It is possible that \texttt{r} aliases a node reachable from \texttt{t}, but even so only the \texttt{v} field of \texttt{r} is written to, and only after the nodes have been copied. The objects allocated to \texttt{rl} and \texttt{rr} do not alias anything, so the recursive calls cannot modify the tree, and hence swapping the order of the recursive calls does not affect the result.

The procedures are not \symdiffequiva{} when the stores are related with equality. With non-deterministic allocation a procedure is not even equivalent to itself! It is conceivable to resolve this with a deterministic allocator, \eg one that starts at 0 and allocates the next address. Under this deterministic approach the procedures are self \symdiffequiva{}, but it does not make \copylr{} and \copyrl{} \symdiffequiva{} --- the allocations on \cref{lst:orig copylr parent} and \cref{lst:orig copyrl parent} are allocated different addresses.
...
%The recursive calls in \cref{fig:copy tree} illustrate challenges \textbf{two, three} and \textbf{four}, here we describe how those challenges are addressed by our implementation.

%\Ctwo{} applies because \textbf{Second}, we do not know in advance which pair of calls are related by an interesting coupling relation. In this example it happens to be that the first call in \copylr{} corresponds to the second call in \copyrl{} and vice versa. But the tool must be able to establish this automatically. \textbf{Third}, because the first call in \copylr{} (\cref{lst:orig copylr call1}) is related to the second call in \copyrl{} (\cref{lst:orig copyrl call2}) the tool has to relate a store from \copylr{} where no recursive call has occurred with a store from \copyrl{} where the effect of a recursive call will have occurred. 

% This definition is a contextual equivalence; \lang{} does not contain any statements that can distinguish between isomorphic stores. In particular, \lang{} programs can not distinguish the actual memory location of an object, nor can they observe the amount or shape of garbage.  We formalise the definitions in \cref{sec:soundness}. 

%Existing tools typically construct an inductive, and modular, argument that procedures are equivalent. \citeauthor{Godlin08}~\cite{Godlin08} and \citeauthor{Lahiri2012}~\cite{Lahiri2012} both describe formal systems extending modular reasoning to pairs of procedures. Such a tool must first establish that the procedures are equivalent for the base cases (in our example, where \texttt{t} or \texttt{r} are null). Then in the inductive cases, a tool may assume that pairs of recursive calls to equivalent procedures with equivalent arguments have equivalent effects. 

In the next section we use the example to illustrate \metho{}, and to illustrate how \metho{} addresses the challenges.

% In \lang{} loops are encoded as recursive procedures. References are opaque, they may be compared for equality or nullness only. To simplify our presentation, values can only be returned via the heap (in this example via the heap cell passed as parameter \texttt{r}). 

\subsection{Contributions}

We propose a sound methodology, \emph{\metho{}}, for establishing isomorphic procedure equivalence which is effective in an SMT solver. \metho{} allows our tool \tool{} (\textbf{A}utomatic \textbf{P}rogram \textbf{E}quivalence tool)\footnote{Source code available at \url{https://github.com/lexicalscope/ape}} to tackle \textbf{\cone{}} by automatically establishing isomorphism using under-approximation and heap equality! \metho{} (\textbf{R}eplace \textbf{I}somorphism with \textbf{E}quality) works by proving equivalence under the \emph{angelic allocation} assumption; the memory locations are, as far as possible, assumed to be allocated in such a way as to make isomorphic heaps also equal. 

We describe a simple language \lang{} for which isomorphism implies equivalence of observable behaviours, and give a formal definition of what it means for a language to be closed under an isomorphism.

\metho{} also simplifies challenges two, three and four. \metho{} allows us to use equality in place of isomorphism, so \textbf{\ctwo{}} is addressed by extending the notion of heap equality to support partial heap equality, which we then apply to an overapproximation of the footprint of the procedures. Furthermore \metho{} rescues us from the need to produce a witness to the correspondence between procedure behaviour, instead we address \textbf{\cthree{}} by equating the write effects of equivalent procedures (soundly ignoring unobservable behavioural differences). We combine our technique with mutual summaries to address \textbf{\cfour{}}.

%This paper contains: A definition of equivalence, and methodology \metho{}, for automatically proving equivalence of programs which vary in dynamic memory allocation. A description of how \metho{} is implemented in an SMT based tool, \tool{}, for equivalence verification of a simple imperative language. A proof of \metho{}'s soundness.

In \cref{sec:impl} we describe how \metho{} can be implemented to verify program equivalence. In \cref{sec:soundness} we formalise equivalence and isomorphism and outline \metho{}'s soundness proof. In \cref{sec:discussion} we discuss the effectiveness and limitations of \metho{}. Finally in \cref{sec:related} we discuss some related work.
...
The framing axioms (\cref{lst:frame axiom lr,lst:frame axiom rl}) restrict the write effects of the procedures to the part of the heap that was reachable from the procedure parameters. The axiom follows from the semantics of \lang{}.%, which does not have pointer arithmetic or pointer synthesis. \Tool{} includes several other framing axioms, but they are beyond the scope of this paper.
...
%	\grammarrow{tr \in \Trace}{ $(s_a,\store,\store)$ | $tr \cdot tr$ } 
\end{tabularx}
...
%The $i^{th}$ stack frame is $\stack[i]$, and $\stack\cdot\stackf$ means the stack with $\stackf$ appended to $\stack$.
...
\lang{} is a simple imperative language. \Cref{fig:bl-op-sem} describes the standard aspects of \lang{}, while \cref{fig:bl call and compose} describes the non-standard aspects. 

The following points are interesting about our semantics: \textbf{First}, we distinguish between execution under $\Vsemantics$ and $\Asemantics$ with a subscript, writing $\exeEither$ where $\metasem \in \{\Vsemantics,\Asemantics\}$. 
\textbf{Second}, we split procedure call into two: a ``call'' rule and a ``body'' rule, similar to \citeauthor{Godlin09}~\cite{Godlin09}, to treat procedure call concretely in $\Vsemantics$ but abstractly in $\Asemantics$. The latter is a first ingredient in reflecting mutual summaries. 
\textbf{Third}, execution of pairs of procedures is included in the operational semantics, modelled by the rules \composeRuleVerified{} and \composeRuleAbstract{}: \[\store_1,s_1 \parcomp \store_2,s_2 \exeEither[\tr_1,\tr_2] \store_3 \parcomp \store_4\] meaning statement $s_1$ executed on store $\store_1$ results in store $\store_3$ producing trace $\tr_1$, and similarly for statement $s_2$. These rules reflect product programs~\cite{Lahiri2012,Barthe2016,Banerjee2016}. 
\textbf{Fourth}, we require the program adhere to a specification given by $\Contract$\footnote{We expect single program contracts to have been verified; programs with no contracts are also acceptable.}.
\textbf{Fifth}, the semantics are instrumented to produce a trace of the states reached during execution which we use to distinguish particular executions.
\textbf{Sixth}, although our semantics is a big step semantics, we keep the whole calling context as part of the runtime configuration to allow us to give useful meaning to isomorphism of stores.
\textbf{Seventh}, we assume loops will been encoded as recursive procedures.
...
\bodyRuleVerified{} executes the body of a procedure \texttt{\fun} by looking up and executing \texttt{\fun}'s statements. Our assumption that procedure contracts have already been verified is modelled by the requirement $(\store_1,\store_3) \in \Contract(\fun)$. Note that \bodyRuleVerified{} pops the top of the final stack, while \callRuleAbstract{} and \callRuleVerified{} push new frames (using the function $\mkframe$). 
...
%The $i^{th}$ stack frame is $\stack[i]$, and $\stack\cdot\stackf$ means the stack with $\stackf$ appended to $\stack$.
%And a mapping between allegedly equivalent procedures $\equivmap : \PName \rightarrow \PName$

%Stuff we don;'t need to say anymore
%We write $\range{R}$ to mean the range of the partial function $R$ and $\domain{R}$ to mean the domain of $R$.
%Infinite enumerable set $\Address$ of addresses, ranged over by $a$, with one reserved element \lstinline{null}. Extended to $\Values \defeq \Address \cup \{\truev, \falsev\}$, ranged over by $v$.
%Enumerable sets $\LVar$ of variable names, $\Field$ of field names, $\PName$ of procedure names, ranged over by $x$, $f$, $f$.
%	
%$\Map \defeq (\Address \times \Field)\parfun\Values$, 
%stack frames $\StackFrame \defeq \LVar\parfun\Values$, 
%and $\Stack \defeq \StackFrame^*$. 
%Ranged over by $h$, $\stackf$ and $\stack$. 
%Where $\forall h,f \suchthat h(\nullv,f)=\nullv$.  
%
%$\Store \defeq \Stack \times \Map$, ranged over by $\store$. The heap of $\store$ is $\heapof{\store}$, and the stack $\stackof{\store}$; The value obtained by following the, possibly empty, sequence of fields $\seq{f}$ from the address in local variable $x$ in the top most stack frame of $\store$ is $\store(x,\seq{f})$.
%
%And $\proc : \PName \rightarrow \Statement$ and $\param : \PName\times\mathbb{N} \parfun \LVar$ , respectively, look up procedure bodies and parameter names from procedure names.


%\begin{definition}[Semantics of \lang{}]\label{def:language}
%
%The statements $\Statement$ and operational semantics are defined in \cref{fig:bl-op-sem,fig:bl call and compose}. The big step execution relation is parameterized by $\metasem \in \{\Vsemantics,\Asemantics\}$ %$\Semantics \defeq \{\Vsemantics,\Asemantics\}$
%			\[\exeEither \subseteq \Store\times(\Statement \cup \body) \rightarrow \Trace\times\Store\] 
%\end{definition}

%Relation $\iso \subseteq \Store \times \powerset{\Values\times\Values} \times \Store$, relates isomorphic stores and is written $\store_1 \iso_{\pi_1} \store_2$, defined in \cref{def:global isomorphism}. 
...
%\metho{} is a modular verification methodology. Modular verification is important in allowing \metho{} to scale to large programs. Modular verification means that, during verification, the behaviour of called procedures is abstracted by a summary. Since \metho{} is concerned with equivalence verification it uses mutual summaries that relate the behaviour of pairs of procedure calls. The summary is assumed in the callers, but verified against the callees. In the case of recursion or mutual recursion an inductive argument is constructed~\cite{Godlin09}.

%As described in \cref{sec:recursion}, \metho{} abstracts pairs of allegedly equivalent procedures with a summary that assumes that allegedly equivalent procedures have \emph{equal} effects. This summary is not an overapproximation of the behaviour of equivalent procedures, because equivalent procedures may differ in the amount and shape of garbage. Nevertheless it is sound. 

%Nevertheless, we would still like to use facts about which called procedures are equivalent. When verifying a particular pair of procedures we assume that the procedures in $\equivmap$ are equivalent, but equivalence is checked for each pair in $\equivmap$ in turn~\cite{Godlin09}.
...
%
%To construct such an execution we will replace the behaviour of all procedure calls according to $\equivmap$ so that the effects are isomorphic. 
%
...
%For instance, it is possible to progress with this example by manually providing an additional mutual summary~\cite{Hawblitzel2013} relating the behaviour of calls to \texttt{find} more precisely.  
...
%One could approach proving equivalence by independently proving functional correctness against the same specification. Many works instead try to use both programs together to simplify the problem. We focus particularly on imperative programs and the state-of-the-art in fully automatic procedure equivalence verifiers. 
...
\emph{Relational Hoare Logic}~\cite{Benton2004,Benton2007,Yang2007,Beringer2011,Barthe2016} (RHL) was proposed by \citeauthor{Benton2004}, in 2004~\cite{Benton2004}, in the course of proving the correctness of various compiler optimisations. The Hoare triple $\{P\} S \{Q\}$ is extended to a Hoare quadruple by inclusion of two statements, rather than one, $\{P\} C_1 \sim C_2 \{Q\}$. The pre and post conditions are lifted to relations over stores. RHL has been extended by various rules to account for differences in structure between the programs~\cite{Benton2004,Barthe2016}. \citeauthor{Barthe2016}~\cite{Barthe2016} pointed out that RHL is closely related to the idea of product programs. % adds additional rules to the logic inspired by the compiler optimisations under examination, \eg eliminating while loops when the condition is known to be false. The connection between RHL and secure information flow is also noted in \citeauthor{Benton2004}{'s} 2004 paper.

 %RHL rules that deal with difference in program structure have close analogues in product program construction rules. RHL can be used to give a relational meaning to the proof on a product program. Many rules in RHL and product program construction allow one program to take a step whilst the other executes a \texttt{skip}, and so have a ``left'' and a ``right'' form depending on which of the programs steps. These rules allow both RHL and product programs to account for some structural difference between the programs. Interestingly the completeness of various RHL variants and product program construction approaches can be related. In particular a self-composition rule can be added to both RHL and product programs to give them the same completeness as the standard Hoare Logic~\cite{Barthe2016}.
