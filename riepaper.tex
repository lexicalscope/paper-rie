...
There are a collection of programs available from \url{https://github.com/lexicalscope/ape#automatic-procedure-equivalence-tool} that show the capabilities of \tool{}. Several of them have been listed in \cref{fig:progs} with timings performed on an {\bf TIM:state the clock speed and the number of cores. The first program gives the timing of an empty program to show the overhead of just calling \tool{} (and Z3).
}
